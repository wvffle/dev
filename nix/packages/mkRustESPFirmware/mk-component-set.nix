# Vendored (with one patch, see below) from
# https://github.com/milas/esp-flake/blob/main/pkgs/rust/mk-component-set.nix
    ...
    stdenvNoCC.mkDerivation rec {
      ...
      postInstall = ''
        ...
        if [[ -e "$out/etc" ]]; then
          ...
          # PATCHED: upstream hard-fails here ("Installer tries to install
          # to /etc: $(ls $out/etc)") on any etc/ file handleEtc doesn't
          # know to relocate. Rust 1.97's tarball ships a new
          # etc/target-spec-json-schema.json we have no use for — discard
          # rather than hard-failing, since new etc/ additions like this
          # are exactly what makes this component installer go stale
          # against newer Rust releases (see esp-rust-toolchain.nix).
          rm -rf "$out/etc"
        fi
      '';
